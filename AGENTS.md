...
- Future RSVP submission notifications must use an external email provider with server-only credentials and recipient configuration. Do not host an SMTP server in the application, and do not discard a saved RSVP when notification delivery fails. Provider selection and delivery implementation remain deferred.
