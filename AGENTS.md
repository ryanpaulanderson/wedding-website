...
- Use Vercel Web Analytics and Speed Insights, mounted once in the root layout. Their client boundary must cancel telemetry when the browser's Do Not Track signal is enabled. Keep telemetry minimal: do not send custom events or properties containing RSVP, admin, or other guest data without an explicit privacy decision.
...
- PostgreSQL with Prisma is the accepted application data foundation: PostgreSQL 17 locally through Docker Compose and separate Neon PostgreSQL resources for Vercel Preview and Production. Publish the local development database only on the host loopback interface, use pooled `DATABASE_URL` connections at runtime, and use `DATABASE_URL_UNPOOLED` for Prisma CLI migrations.
- Store optional dietary restrictions per named guest and per plus-one. Each named guest has at most one plus-one, permitted only by the maintainer's upload setting (`plusOneAllowed`, default false); future guest submissions must never change that permission. Store the plus-one's name, attendance, and dietary restrictions separately from the named guest, and require a named, attending plus-one to accompany an attending invited guest. Future RSVP counts must include plus-one places and attendance explicitly.
- Support future RSVP access by private household invitation link and name lookup. Persist only the lowercase SHA-256 hash of a high-entropy invitation token, never the usable token. Name-lookup authorization and rate limiting must be designed before implementing the lookup endpoint; the model alone does not authorize guest access.
- Send maintainer RSVP notifications through Resend's HTTPS API using server-only `RESEND_API_KEY` and `RESEND_EMAIL_DOMAIN`. Encrypt the entire body with the recipient's pinned public OpenPGP key before calling Resend; keep the recipient, public key, and fingerprint together in the server-only notification configuration. Never store private keys or send plaintext fallbacks, guest details in headers, HTML alternatives, or guest-data telemetry. Use generic subjects and allow delivery in any environment with valid Resend configuration, including Preview for end-to-end testing. Previously paused email jobs must be retryable from admin. Automated tests must mock the provider or explicitly remove mail credentials; do not gate application delivery on the environment name. Keep each retry's ciphertext and idempotency key identical. Use the early-notice durable outbox pattern for retries across requests and never discard a saved response because email failed.
- Keep early unable-to-attend notices separate from formal household RSVPs. Require freeform names and a normalized unique email; duplicates must not overwrite a notice or resend mail. Save the notice and its guest-confirmation/admin-notification jobs atomically. Guest confirmation uses fixed text and Ryan's reply address; admin notice bodies remain PGP-encrypted. Persist identical email payloads before sending, lease concurrent delivery, never resend accepted jobs, and stop uncertain retries before Resend's 24-hour idempotency expiry. Use the database-backed five-attempt hourly client limit with a purpose-separated HMAC of the trusted client identity; keep raw IPs out of storage. Reauthorize all admin review and retry actions independently.
- Commit Prisma migrations and apply them to hosted databases only through an explicit maintainer-run `prisma migrate deploy`. Never run migrations at application startup or on every Vercel build, never use `prisma migrate dev` or `prisma db push` against Preview or Production, and keep hosted schema changes backward-compatible with the currently deployed application.
- Store deployment secrets and connection strings in environment variables configured per environment. Never commit production values, expose them through `NEXT_PUBLIC_`, or allow Preview deployments to access the Production database or guest data.
...
- Keep the removable hosted-site password gate active for guests using the password distributed on the Save the Date to help reduce web scraping. Protect every Vercel deployment and bypass it for all local execution. Centralize enablement in `isSitePasswordGateEnabled()`; never read `SITE_PASSWORD_GATE` elsewhere or treat the shared password as RSVP authorization. The access page must explain where to find the password and direct guests who need help to Ryan at `ryan@ryanpaulanderson.com`.
...
- Keep `/admin` unlinked and independently protected on every Vercel deployment with dedicated admin credentials. Bypass admin authentication for all local execution, including local production builds, through `isAdminAuthenticationRequired()` using the server-only `VERCEL` environment marker; never use request headers or hostnames to enable the bypass. Local dashboards must not show sign-out controls. Reauthorize inside every admin data read and mutation through the central environment-aware helper, and keep password verification bounded by both application and edge rate limits; the temporary site password, page visibility, and `proxy.ts` are never sufficient admin authorization.
