...
- Send maintainer RSVP notifications through Resend's HTTPS API using server-only `RESEND_API_KEY` and `RESEND_EMAIL_DOMAIN`. Encrypt the entire body with the recipient's pinned public OpenPGP key before calling Resend; keep the recipient, public key, and fingerprint together in the server-only notification configuration. Never store private keys or send plaintext fallbacks, guest details in headers, HTML alternatives, or guest-data telemetry. Use generic subjects and disable automatic delivery outside Production; only explicit maintainer test commands may override this. Keep each retry's ciphertext and idempotency key identical. Before connecting the deferred submission flow, add a durable encrypted outbox for retries across requests and never discard a saved RSVP because email failed.
